...
export default class BandCampSource {
    ...
    async search(query) {
        try {
            const request = await makeRequest(`${this.baseUrl}/search?q=${encodeURI(query)}&item_type=t&from=results`, { method: 'GET' });
    ...
    async getTrackUrl(track) {
        try {
            ...
            const streamUrlMatch = body.match(/https?:\/\/t4\.bcbits\.com\/stream\/[a-zA-Z0-9]+\/mp3-128\/\d+\?p=\d+&amp;ts=\d+&amp;t=[a-zA-Z0-9]+&amp;token=\d+_[a-zA-Z0-9]+/);
            if (!streamUrlMatch) {
    ...
    async loadStream(decodedTrack, url, _protocol, _additionalData) {
        ...
        try {
            ...
            logger('error', 'Sources', `Failed to load BandCamp stream: ${err.message}`);
            return {
                exception: {
                    message: err.message,
                    severity: 'common',
    ...
    async extractTralbumData(url) {
        ...
        if (error || statusCode !== 200) {
            logger('error', 'Sources', `Failed to fetch BandCamp page: ${error?.message || statusCode}`);
            return null;
        }
