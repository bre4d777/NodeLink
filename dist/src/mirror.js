const log = {
    info: (msg) => console.log(`\x1b[32m[INFO]\x1b[0m  [Mirroring] ${msg}`),
    debug: (msg) => console.log(`\x1b[34m[DEBUG]\x1b[0m [Mirroring] ${msg}`),
    warn: (msg) => console.log(`\x1b[33m[WARN]\x1b[0m  [Mirroring] ${msg}`),
};
...
function durationSimilarity(d1, d2) {
    if (d1 <= 0 || d2 <= 0)
        return 0.5;
...
function scoreMatch(original, candidate) {
    ...
    const artistScore = stringSimilarity(original.author || '', candidate.info?.author || candidate.author || '');
    const durScore = durationSimilarity(original.length || 0, candidate.info?.length || candidate.length || 0);
...
async function validateStream(nodelink, match) {
...
async function findBestValidMatch(nodelink, scoredMatches, threshold) {
...
async function mirror(nodelink, track, mirroringSources) {
    const sources = mirroringSources ?? ['ytmsearch', 'ytsearch'];
    if (sources.length === 0) {
    ...
    for (let i = 0; i < sources.length; i++) {
        ...
        const result = await findBestValidMatch(nodelink, ranked.slice(0, top >= HIGH_CONFIDENCE ? 2 : 3), top >= HIGH_CONFIDENCE ? HIGH_CONFIDENCE : MIN_SIMILARITY);
        if (result) {
            if (!globalBest || result.score > globalBest.score + 0.08)
                globalBest = { ...result, prefix };
