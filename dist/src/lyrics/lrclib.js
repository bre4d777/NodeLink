...
export default class LRCLIBLyrics {
    ...
    async getLyrics(trackInfo) {
        ...
        try {
            const { body: results } = await makeRequest(`https://lrclib.net/api/search?q=${encodeURIComponent(query)}`, { method: 'GET' });
            ...
            let bestMatch = null;
            bestMatch = results.find((r) => _clean(r.trackName, true).toLowerCase() === titleLower &&
            ...
            logger('error', 'Lyrics', `Failed to fetch lyrics from LRCLIB: ${e.message}`);
            return {
                loadType: 'error',
